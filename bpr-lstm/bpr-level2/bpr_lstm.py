...
class PMF(nn.Module):
    def __init__(self, n_users, n_items, n_factors=30, is_sparse=False, no_cuda=False):
        ...
        self.U.weight.data = torch.from_numpy(0.01 * self.random_state.standard_normal(size = (n_users, n_factors))).float()
        # V: song's latent features for interests
        self.V = nn.Embedding(n_items, n_factors, sparse=is_sparse)
        self.V.weight.data = torch.from_numpy(0.01 * self.random_state.standard_normal(size = (n_items, n_factors))).float()
        # C: user's latent features for performance 
        self.C = nn.Embedding(n_users, n_factors, sparse=is_sparse)
        self.C.weight.data = torch.from_numpy(0.01 * self.random_state.standard_normal(size = (n_users, n_factors))).float()
        # R: user's latent features for feedbacks of performance
        self.R = nn.Embedding(n_users, n_factors, sparse=is_sparse)
        self.R.weight.data = torch.from_numpy(0.01 * self.random_state.standard_normal(size = (n_users, n_factors))).float()
        # D: song's latend features for performance
        self.D = nn.Embedding(n_items, n_factors, sparse=is_sparse)
        self.D.weight.data = torch.from_numpy(0.01 * self.random_state.standard_normal(size = (n_items, n_factors))).float()
        
        self.alpha = nn.Embedding(n_users, 1, sparse=is_sparse)
        self.alpha.weight.data = torch.from_numpy(np.array([0.5] * n_users)).float()
